...
test.describe("Page Tour - High Quality Recording", () => {
  test.beforeEach(async () => {
    logger.start();
  });
  ...
  test("tour all main pages", async ({ page }) => {
    // Setup page
    logger.stepStart(1, "Setup Page");
    await page.goto("/setup");
    logger.navigate("/setup");
    await expect(page.getByRole("heading", { name: /baseline/i })).toBeVisible();
    logger.assertPass("Setup page loaded");
    await page.waitForTimeout(2000);

    // Manager briefing
    logger.stepStart(2, "Manager Briefing");
    await page.goto("/manager/briefing");
    logger.navigate("/manager/briefing");
    await expect(page.getByRole("heading", { name: /daily briefing/i })).toBeVisible();
    logger.assertPass("Briefing page loaded");
    await page.waitForTimeout(2000);

    // Manager dashboard
    logger.stepStart(3, "Manager Dashboard");
    await page.goto("/manager");
    logger.navigate("/manager");
    await page.waitForTimeout(2000);
    logger.assertPass("Manager dashboard loaded");

    // Admin page
    logger.stepStart(4, "Admin Dashboard");
    await page.goto("/admin");
    logger.navigate("/admin");
    await page.waitForTimeout(2000);
    logger.assertPass("Admin page loaded");

    // Staff page
    logger.stepStart(5, "Staff Page");
    await page.goto("/staff");
    logger.navigate("/staff");
    await page.waitForTimeout(2000);
    logger.assertPass("Staff page loaded");

    // Scoreboard
    logger.stepStart(6, "Scoreboard");
    await page.goto("/scoreboard");
    logger.navigate("/scoreboard");
    await page.waitForTimeout(2000);
    logger.assertPass("Scoreboard loaded");
  });
});
