...
import { ScanLine, Check, MessageSquarePlus, TrendingUp, Zap, Sparkles, X, ArrowRight, CheckCircle, Plus } from 'lucide-react';
import { clsx } from 'clsx';

export default function StaffPage() {
  const { staff, behaviors, logBehavior, entries, currentDate } = useApp();
  
  // Mock login (select current user)
  const [selectedStaffId, setSelectedStaffId] = useState<string>('s1'); 
  const currentUser = staff.find(s => s.id === selectedStaffId);
  ...
  const myStats = todayEntry?.staffStats.find(s => s.staffId === selectedStaffId);
  const myTotalBehaviors = myStats 
    ? Object.values(myStats.behaviorCounts).reduce((a, b) => a + b, 0) 
    : 0;
  ...
  const handleSubmitLog = () => {
    if (selectedBehavior) {
      logBehavior(selectedStaffId, selectedBehavior, {
      ...
      setCheckAmt('');
      setView('dashboard');
      // Show success toast logic here (omitted for brevity)
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white pb-32 font-sans">
      ...
      {/* Staff Selector Header (Demo) */}
      <div className="bg-slate-800/50 backdrop-blur p-4 overflow-x-auto flex gap-4 border-b border-slate-700 sticky top-0 z-20">
        {staff.filter(s => s.role === 'staff').map(s => (
          <button
            ...
            className={clsx(
              ...
              selectedStaffId === s.id ? "opacity-100 scale-110" : "opacity-40 hover:opacity-70"
            )}
          >
            <div className="h-10 w-10 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center font-bold text-white shadow-lg mb-1 text-xs">
              {s.avatar}
            </div>
          </button>
        ))}
            ...
            >
              <div className="bg-slate-900/40 rounded-lg p-4 flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-white">"Hey {currentUser?.name}, try suggesting the 'Chef Special' to Table 4. It pairs well with their wine."</p>
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      if (isConfirming) {
                        // Confirmed
                        logBehavior(selectedStaffId, b.id, {});
                  ...
                  >
                    <div className="flex justify-between items-center relative z-10">
                      ...
                      <div className={clsx(
                        ...
                        {isConfirming ? <Check size={20} /> : <Plus size={16} />}
                      </div>
                    </div>
